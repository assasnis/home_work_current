...
def create_voting_dict(strlist):
    ...
    return {item[0]:[int(ii) for ii in item[3:]] for item in [each_string.split() for each_string in strlist]}
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum([voting_dict[sen_a][ii]*voting_dict[sen_b][ii] for ii in range(len(voting_dict[sen_a]))])
...
def most_similar(sen, voting_dict):
    ...
    """
    pol_comp={policy_compare(sen, sen_x, voting_dict):sen_x for sen_x in voting_dict.keys() if sen_x!=sen}
    
    return pol_comp[max(pol_comp.keys())]
...
def least_similar(sen, voting_dict):
    ...
    return pol_comp[min(pol_comp.keys())]
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        True
    """
    list_similarity=[policy_compare(sen,item,voting_dict) for item in sen_set]
    return float(sum(list_similarity))/len(list_similarity)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    #sum_lists=[voting_dict[sen][jj]+voting_dict[next_sen][jj] for sen in sen_set]
    #a=[ii*0 for ii in range 
