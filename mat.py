...
def getitem(M, k):
    ...
    return [M.f[(k[0],k[1])] if (k[0],k[1]) in M.f.keys() else 0][0]
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D,{key:val*x for key,val in zip(M.f.keys(),M.f.values())})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1],M.D[0]),{(key[1],key[0]):val for key,val in zip(M.f.keys(),M.f.values())})
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0],B.D[1]),{(rowA,colB):Vec(A.D[1],{colA:A[rowA,colA] for colA in A.D[1]})*Vec(B.D[0],{rowB:B[rowB,colB] for rowB in B.D[0]}) for colB in B.D[1] for rowA in A.D[0]})
