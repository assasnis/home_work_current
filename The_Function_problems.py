...
def tuple_sum(A, B):
    ...
    [(4, 6), (40, 60)]
    '''
    return [(lA[0]+lB[0],lA[1]+lB[1])for lA,lB in zip(A,B)]
...
def inv_dict(d):
    ...
    '''
    return {y:x for x,y in zip(d.keys(),d.values())}
...
def row(p, n):
    ...
    [10, 11, 12, 13]
    '''
    return [ii for ii in range(p,p+n)]


comprehension_with_row =  [row(ii,20) for ii in range(15)]



comprehension_without_row = [[i for i in range(ii,ii+20)] for ii in range(15)]
