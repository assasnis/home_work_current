...
value_list = [dlist[ii][k] for ii in range(len(dlist))]
...
value_list_modified_1 = [dlist[ii].get(k,'NOT PRESENT') for ii in range(len(dlist))] # <-- Use the same expression here
k = 'Frodo'
value_list_modified_2 = [dlist[ii].get(k,'NOT PRESENT') for ii in range(len(dlist))] # <-- as you do here



## 22: (Task 22) A dictionary mapping integers to their squares
# Replace {...} with a one-line dictionary comprehension
square_dict = {ii:ii**2 for ii in range(100)}
...
D = {'red','white','blue'}
# Replace {...} with a one-line dictionary comprehension
identity_dict = {ii:ii for ii in D}
...
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def dict2list(dct, keylist): return [ dct.get(y) for y in keylist ]
