...
def add(u,v):
    ...
    """
    assert u.D == v.D
    [a,b]=[{item:getitem(u,item) for item in u.D},{item:getitem(v,item) for item in v.D}]
    return Vec(u.D,{item:(a[item]+b[item]) for item in u.D })
...
def dot(u,v):
    ...
    assert u.D == v.D
    [a,b]=[{item:getitem(u,item) for item in u.D},{item:getitem(v,item) for item in v.D}]
    return sum([a[item]*b[item] for item in u.D])
...
def scalar_mul(v, alpha):
    ...
    """
    new_Vec=Vec(v.D,{})
    new_Vec.f.update({item:v.f[item]*alpha for item in v.f.keys() if alpha!=0})
    return new_Vec 
...
def neg(v):
    ...
    """
    new_Vec=Vec(v.D,{})
    new_Vec.f.update({item:v.f[item]*-1 for item in v.f.keys()})
    return new_Vec 
